...
test.describe('Task Deletion Feature', () => {
  ...
  test('instructor can see delete button for tasks with no student progress', async ({ page }) => {
    ...
    const firstCourse = page.locator('[data-testid^="course-list-item-"], [data-testid^="course-card-"]').first();
    ...
    if (courseIdAttr) {
      ...
      if (infoCount > 0) {
        ...
        if (tooltipText) {
          expect(tooltipText).toMatch(/Cannot delete:|student\(s\) in progress|student\(s\) completed/);
        }
      }
    }
  });
  ...
  test('instructor can delete a task with confirmation dialog', async ({ page }) => {
    ...
    if (courseIdAttr) {
      ...
      await page.fill('[name="description"], [data-testid="task-description-input"]', 'This task will be deleted');
      ...
      if (await deleteButton.isVisible()) {
        ...
        const confirmButton = dialog.locator('[data-testid="confirm-delete-task"], button:has-text("Delete")').last();
  ...
  test('student cannot see delete buttons for tasks', async ({ page }) => {
    ...
    const firstCourse = page.locator('[data-testid^="course-"], .course-card, .course-list-item').first();
