...
export class DashboardPage extends BasePage {
  ...
  constructor(page: Page, basePath = '/dashboard') {
    super(page, basePath);
    this.dashboardSummary = this.page.locator(
      ...
      await this.page.waitForURL(
        url => {
          ...
          return (
            ...
            pathname === '/'
          ); // Fallback für die Hauptseite nach Login
        },
        { timeout: 10000 }
      );
  ...
  async hasCourseCards(): Promise<boolean> {
    const courseCards = this.page.locator(
      '.course-card, [data-testid="course-card"], .MuiPaper-root:has(.MuiCardContent-root)'
    );
  ...
  async hasSpecificCourse(courseTitle: string): Promise<boolean> {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const title = await card
        .locator('h2, h3, .course-title, [data-testid="course-title"]')
        .textContent();
  ...
  async hasProgressSection(): Promise<boolean> {
    const progressSection = this.page.locator(
      '.progress-section, [data-testid="progress-section"], .progress-chart'
    );
  ...
  async getNavigationItems(): Promise<string[]> {
    ...
    try {
      ...
      const navContainer = await this.findElement(this.navigationMenuSelectors, 'navigation menu', {
        timeoutMs: 5000,
      });
    ...
    try {
      ...
      for (const [index, element] of dashboardElements.entries()) {
        try {
          ...
          console.log(
            `Selector "${selectorTexts[index]}": Found ${count} elements, visible: ${isVisible}`
          );
...
export class StudentDashboardPage extends DashboardPage {
  ...
  async getEnrolledCourses(): Promise<string[]> {
    ...
    try {
      ...
      const courseElements = this.page.locator(
        '.course-card, .course-list-item, [data-testid^="course-"]'
      );
  ...
  override async waitForPageLoad(timeoutMs: number = 10000): Promise<void> {
    try {
      ...
      try {
        await this.page.waitForSelector('.course-card, [data-testid="no-courses-message"]', {
          timeout: 2000,
        });
      } catch {
...
export class InstructorDashboardPage extends DashboardPage {
  ...
  async navigateToCreateCourse(): Promise<void> {
    ...
    try {
      ...
      console.log(
        'Could not find create course button, navigating directly to /instructor/courses/new'
      );
...
export class AdminDashboardPage extends DashboardPage {
  ...
  readonly adminStatsSelectors = ['.admin-stats', '.platform-stats', '[data-testid="admin-stats"]'];
  ...
  async navigateToUserManagement(): Promise<void> {
    try {
      const userManagementLink = await this.findElement(
