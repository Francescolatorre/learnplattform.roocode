...
from learning.models import Course, CourseInstructorAssignment, InstructorRole, CourseVersion
...
class TestUserAcceptanceCourseManagement:
    ...
    def test_course(self, test_users, instructor_roles):
        ...
        CourseInstructorAssignment.objects.create(
            course=course,
            instructor=test_users['lead'],
            role=instructor_roles['LEAD']
        )
        CourseInstructorAssignment.objects.create(
            course=course,
    ...
    def test_instructor_role_permissions(self, test_course, test_users, instructor_roles):
        """
        ...
        assert test_course.has_instructor_permission(test_users['lead'], 'edit_course') == True
        assert test_course.has_instructor_permission(test_users['lead'], 'manage_tasks') == True
        assert test_course.has_instructor_permission(test_users['lead'], 'grade_submissions') == True
        ...
        assert test_course.has_instructor_permission(test_users['assistant'], 'edit_course') == False
        assert test_course.has_instructor_permission(test_users['assistant'], 'manage_tasks') == True
        assert test_course.has_instructor_permission(test_users['assistant'], 'grade_submissions') == True
        ...
        assert test_course.has_instructor_permission(test_users['guest'], 'edit_course') == False
        assert test_course.has_instructor_permission(test_users['guest'], 'manage_tasks') == False
        assert test_course.has_instructor_permission(test_users['guest'], 'grade_submissions') == False
    ...
    def test_course_version_creation(self, test_course, test_users):
        ...
        new_version = version_service.create_version(
            user=test_users['lead'], 
            notes='First version update'
        )
    ...
    def test_version_rollback(self, test_course, test_users):
        ...
        version_service.create_version(
            user=test_users['lead'], 
            notes='Version 2 changes'
        )
        version_service.create_version(
            user=test_users['lead'], 
            notes='Version 3 changes'
        )
