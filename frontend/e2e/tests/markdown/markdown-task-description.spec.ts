...
test.describe('Learning Task Description Markdown Rendering', () => {
  ...
  test('student can view learning task with markdown description', async ({ page }) => {
    ...
    await MarkdownTestUtils.verifyMarkdownRendering(page, '.task-description', {
      headers: true,
      paragraphs: true,
      lists: true,
      codeBlocks: true,
  ...
  test('unsafe markdown content is sanitized', async ({ page }) => {
    ...
    const loginPage = new LoginPage(page);
    await loginPage.navigateTo();
    await loginPage.login(TEST_USERS.lead_instructor.username, TEST_USERS.lead_instructor.password); // Navigate to course tasks page
    await page.goto(`/instructor/courses/${courseId}/tasks`);
