...
describe('learningTaskService Integration', () => {
    ...
    it('getAll returns learning tasks', async () => {
        ...
        */
        ...
        if (!found) {
            console.error('Created task not found in getAll response. CreatedTaskId:', baseTaskId, 'Returned IDs:', tasks.map((t: any) => t.id));
        }
    ...
    it('update updates the learning task title', async () => {
        const updated = await learningTaskService.update(String(baseTaskId), {title: 'Updated Integration Test Task'});
