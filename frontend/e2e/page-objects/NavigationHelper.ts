...
export class NavigationHelper extends BasePage {
    ...
    readonly desktopNavSelectors = {
        coursesButton: 'header button:has-text("Courses"), .MuiAppBar-root button:has-text("Courses"), [data-testid="nav-courses"]',
        dashboardButton: 'header button:has-text("Dashboard"), .MuiAppBar-root button:has-text("Dashboard"), [data-testid="nav-dashboard"]',
        profileButton: 'header button:has-text("Profile"), .MuiAppBar-root button:has-text("Profile"), [data-testid="nav-profile"]',
        instructorButton: 'header button:has-text("Instructor"), .MuiAppBar-root button:has-text("Instructor"), [data-testid="nav-instructor"]',
        adminButton: 'header button:has-text("Admin"), .MuiAppBar-root button:has-text("Admin"), [data-testid="nav-admin"]',
        loginButton: 'header button:has-text("Login"), .MuiAppBar-root button:has-text("Login"), [data-testid="nav-login"]',
        registerButton: 'header button:has-text("Register"), .MuiAppBar-root button:has-text("Register"), [data-testid="nav-register"]',
        logoutButton: 'header button:has-text("Logout"), .MuiAppBar-root button:has-text("Logout"), [data-testid="nav-logout"]'
    };

    // Mobile drawer navigation selectors
    readonly mobileNavSelectors = {
        menuButton: '.MuiIconButton-root[aria-label="open drawer"], button:has(.MuiSvgIcon-root), [data-testid="menu-button"]',
        drawer: '.MuiDrawer-root, [role="presentation"]',
        coursesItem: '.MuiDrawer-root [role="presentation"] a:has-text("Courses"), .MuiDrawer-paper a:has-text("Courses")',
        dashboardItem: '.MuiDrawer-root [role="presentation"] a:has-text("Dashboard"), .MuiDrawer-paper a:has-text("Dashboard")',
        profileItem: '.MuiDrawer-root [role="presentation"] a:has-text("Profile"), .MuiDrawer-paper a:has-text("Profile")',
        instructorItem: '.MuiDrawer-root [role="presentation"] a:has-text("Instructor"), .MuiDrawer-paper a:has-text("Instructor")',
        adminItem: '.MuiDrawer-root [role="presentation"] a:has-text("Admin"), .MuiDrawer-paper a:has-text("Admin")',
        logoutItem: '.MuiDrawer-root [role="presentation"] li:has-text("Logout"), .MuiDrawer-paper li:has-text("Logout")'
    };
    ...
    async ensureAuthenticated(): Promise<void> {
        ...
        if (!isAuthenticated) {
            ...
            if (this.userRole) {
                ...
                if (!success) {
                    throw new Error(`Authentication as ${this.userRole} failed. Cannot proceed with navigation.`);
                }
                this.authenticated = true;
            } else {
                throw new Error('Authentication verification failed. Cannot navigate when unauthenticated.');
            }
        }
    }
    ...
    async isMobileView(): Promise<boolean> {
        return await this.page.locator(this.mobileNavSelectors.menuButton).isVisible()
            .catch(() => false);
    }
    ...
    async navigateToInstructorCourses(): Promise<void> {
        ...
        const coursesLink = this.page.locator('a:has-text("Courses"), [href*="courses"], button:has-text("Courses")').first();
     ...
     */
    private async clickNavigationItem(item: 'courses' | 'dashboard' | 'profile' | 'instructor' | 'admin'): Promise<void> {
        const isMobile = await this.isMobileView();
