...
import { loginAsStudent, loginAsInstructor, loginAsAdmin } from '../utils/auth-helpers';
import {
  logPageState,
  debugElement,
...
test.describe('Utility Functions Examples', () => {
  ...
  test('Example: Basic Login Function', async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="user-menu"], .user-avatar, header .username')
    ).toBeVisible();
  ...
  test('Example: Markdown Test Utilities', async ({ page }) => {
    ...
    console.log(
      'Available markdown container selectors:',
  ...
  test('Practical Example: Course Description Workflow', async ({ page }) => {
    test.info().annotations.push({
      type: 'example',
      description:
        'A practical example combining multiple utilities for testing a course description workflow',
    });
    ...
    if (await courseCard.isVisible()) {
      ...
      try {
        await MarkdownTestUtils.verifyMarkdownRendering(
          page,
          '.course-description, [data-testid="course-description"]'
        );
