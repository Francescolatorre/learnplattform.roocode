...
const StudentCourseDetailsPage: React.FC = () => {
  ...
  const canViewTasks = useMemo(() => {
    if (!courseDetails || !enrollmentStatus) return false;
  ...
  const {
    ...
    enabled: !!courseIdNum && canViewTasks,
    retry: false,
            ...
            learningTasks.results && learningTasks.results.length > 0 ? (
              <List>
                {learningTasks.results.map(task => (
                  <ListItem key={task.id} divider data-testid={`task-item-${task.id}`}>
                    <ListItemText
                      primary={<span data-testid={`task-title-${task.id}`}>{task.title}</span>}
      ...
      >
        ...
        <DialogActions>
          ...
          <Button
            ...
            color="secondary"
            data-testid="confirm-unenroll-button"
            autoFocus
          >
            Unenroll
          </Button>
