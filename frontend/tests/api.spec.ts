...
test.describe('API Authentication and Authorization', () => {
  ...
  test('Admin can authenticate and receive valid tokens', async () => {
    const tokens = await apiHelper.authenticate(TEST_USERS.admin.username, TEST_USERS.admin.password);
  ...
  test('Admin can access courses list', async () => {
    ...
    expect(Array.isArray(responseData) || (responseData.results && Array.isArray(responseData.results))).toBeTruthy();
  });
  ...
  test('Student can access courses list but cannot modify courses', async () => {
    ...
    } else if (coursesData.results && Array.isArray(coursesData.results) && coursesData.results.length > 0) {
      courseId = coursesData.results[0].id;
    } else {
      test.skip('No courses found to test permissions');
      return;
    }
    ...
    expect(createTaskResponse.status()).toBeOneOf([401, 403]);
  });
  ...
  test('Admin can create, update and delete tasks', async () => {
    ...
    if (Array.isArray(coursesData) && coursesData.length > 0) {
      ...
      test.skip('No courses found to test task operations');
      return;
    }
    ...
    const requestContext = await request.newContext({
      baseURL: API_URL,
      extraHTTPHeaders: {
        'Authorization': `Bearer ${apiHelper.accessToken}`
      }
    });
