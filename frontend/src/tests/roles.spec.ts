...
class UserSession {
  ...
  async login() {
    ...
    await this.page.waitForSelector(
      'input[name="username"], input[name="username_or_email"], [data-testid="username-input"]',
      { timeout: 30000 }
    );
    ...
    const loginButton = this.page.locator(
      'button[type="submit"], [data-testid="login-button"], button:has-text("Sign In"), button:has-text("Login")'
    );
  ...
  async navigateToFirstCourse() {
    ...
    await this.page.waitForSelector('[data-testid="course-card"], .MuiCard-root', {
      timeout: 10000,
    });
