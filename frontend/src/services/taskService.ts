...
export const fetchTasksByCourse = async (courseId: string, includeSubtasks = false): Promise<{ count: number; next: string | null; previous: string | null; results: Task[] }> => {
  return apiService.get(`tasks/course/${courseId}/`, { includeSubtasks });
};

export const createTask = async (taskData: TaskCreationData, notifyUsers = false): Promise<Task> => {
...
export const fetchTaskSubmissions = async (taskId: string): Promise<{ count: number; next: string | null; previous: string | null; results: any[] }> => {
