...
export class BaseCourses extends BasePage {
  ...
  protected readonly courseCardSelectors = ['[data-testid^="course-card-"]', '.course-card'];

  protected readonly courseGridSelectors = ['[data-testid="courses-grid"]', '.courses-grid'];
  ...
  async filterByStatus(status: '' | 'draft' | 'published' | 'archived'): Promise<void> {
    try {
      ...
      const option = this.page.locator(
        `.MuiMenu-list [data-value="${status}"], .MuiMenuItem-root[data-value="${status}"]`
      );
      await option.click();
