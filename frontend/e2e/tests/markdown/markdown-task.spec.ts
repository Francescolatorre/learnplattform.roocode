...
test.describe('Learning Task Description Markdown Rendering', () => {
  ...
  test.beforeAll(async ({ browser }) => {
    ...
    try {
      ...
      await page
        .locator('#course-description, textarea[name="description"]')
        .fill('Course for testing task markdown rendering');
      ...
      await page
        .waitForURL(/\/instructor\/courses\/\d+/, { timeout: 10000 })
      ...
      if (courseIdMatch && courseIdMatch[1]) {
        ...
        const courseIdText = await page
          .locator('[data-testid="course-id"]')
          .textContent()
          .catch(() => null);
        if (courseIdText) {
      ...
      await page
        .getByRole('button', { name: /Add Task|Create Task|New Task/ })
        .first()
        .click()
  ...
  test('instructor can create a learning task with markdown description', async ({ page }) => {
    ...
    const successVisible = await page
      .getByText(/Task created successfully|Task saved successfully|saved successfully/)
      .isVisible()
      .catch(() => false);
    const returnedToTaskList = page.url().includes('/tasks');

    console.log(
      `Success message visible: ${successVisible}, Returned to task list: ${returnedToTaskList}`
    );
  ...
  test('task list shows markdown preview correctly', async ({ page }) => {
    ...
    if (!taskExists) {
      ...
      const anyTaskExists = await page
        .locator(taskSelector)
        .first()
        .isVisible()
        .catch(() => false);
    ...
    const descriptionContent = await taskItem
      .locator('.task-description, .description, p')
      .innerHTML()
      .catch(() => '');
  ...
  test('student can view learning task with markdown description', async ({ page }) => {
    ...
    const taskNavExists = await page
      .getByRole('button', { name: /View Tasks|Continue Learning|Tasks/ })
      .isVisible()
      .catch(() => false);
    ...
    try {
      await MarkdownTestUtils.verifyMarkdownRendering(page, descriptionSelector, {
        headers: true,
        paragraphs: true,
        lists: true,
        codeBlocks: true,
  ...
  test('unsafe markdown content is sanitized in learning tasks', async ({ page }) => {
    ...
    const taskItem = page
      .locator('.task-item, li')
      .filter({ hasText: 'Unsafe Markdown Test Task' })
      .first();
    ...
    // Verify sanitization is working properly
    try {
      await MarkdownTestUtils.verifySanitization(
        page,
        '.task-description, [data-testid="task-description"]'
      );
