...
export class CourseCreationPage extends BasePage {
  ...
  readonly validationErrorSelectors = ['.Mui-error', '.error-message', '.validation-error'];
  ...
  async fillCategory(category: string): Promise<void> {
    try {
      const categoryInput = await this.findElement(this.categoryInputSelectors, 'category input', {
        timeoutMs: 1000,
      }).catch(() => null);
  ...
  async selectDifficulty(difficulty: 'beginner' | 'intermediate' | 'advanced'): Promise<void> {
    try {
      const difficultySelect = await this.findElement(
  ...
  async fillImageUrl(url: string): Promise<void> {
    try {
      const imageUrlInput = await this.findElement(this.imageUrlInputSelectors, 'image URL input', {
        timeoutMs: 1000,
      }).catch(() => null);
  ...
  async setPublishStatus(isPublished: boolean): Promise<void> {
    try {
      const publishSwitch = await this.findElement(this.publishSwitchSelectors, 'publish switch', {
        timeoutMs: 1000,
      }).catch(() => null);
  ...
  async checkValidationErrors(): Promise<{
    ...
    try {
      ...
      if (titleInput) {
        ...
        result.titleError = await titleInput.evaluate(
      ...
      const descInput = await this.findElement(
      ...
      if (descInput) {
        result.descriptionError = await descInput
  ...
  private async getFieldErrorMessage(fieldLocator: Locator): Promise<string | null> {
    try {
      ...
      if (fieldId) {
        const errorByAriaDescribedby = this.page.locator(
          `[id="${fieldId}-error"], [id="${fieldId}-helper-text"]`
        );
  ...
  async waitForSuccessNotification(
    ...
    try {
        ...
        try {
          ...
          if (
            ...
            return {
              success: true,
              courseId,
              message: 'Course created successfully (detected via URL change)',
            };
          }
        } catch (urlError) {
      ...
      await this.page
        .waitForURL('**/instructor/courses/*', { timeout: timeoutMs })
  ...
  async validateCourseDetails(courseId: string, expectedTitle: string): Promise<boolean> {
    try {
      ...
      if (!currentUrl.includes(courseId)) {
        console.error(
          `Not on the expected course page. Expected URL to contain ${courseId}, but got ${currentUrl}`
        );
        return false;
      }
      ...
      const hasEditOption = await this.page
        .getByText('Edit Course')
        .isVisible()
        .catch(() => false);

      console.log(
        `Course validation results - Title visible: ${titleVisible}, Edit option: ${hasEditOption}`
      );
  ...
  async verifyOptimisticUpdate(courseData: ICourseCreationData): Promise<boolean> {
    try {
      ...
      if (
        currentUrl.includes('/instructor/courses/new') ||
        currentUrl.includes('/instructor/courses/edit')
      ) {
        ...
        const descMatch =
          !courseData.description || formValues.description === courseData.description;
        console.log(
          `Standalone page optimistic update check: Title match=${titleMatch}, Description match=${descMatch}`
        );
      ...
      const tempTitleExists = await this.page
        .locator(this.optimisticUpdateSelectors.temporaryTitle)
        .isVisible()
        .catch(() => false);

      if (!tempTitleExists) {
        console.log(
          'Optimistic update UI elements not found, checking for course in the list instead'
        );
  ...
  async waitForOptimisticUpdateConfirmation(timeout = 5000): Promise<void> {
    ...
    const loadingIndicatorExists = await this.page
      .locator(this.optimisticUpdateSelectors.loadingIndicator)
  ...
  async hasValidationErrors(): Promise<boolean> {
    try {
      ...
      const titleErrorState = await this.page
        .locator('input[name="title"][aria-invalid="true"]')
        .isVisible()
        .catch(() => false);

      const descriptionErrorState = await this.page
        .locator('textarea[aria-invalid="true"]')
        .isVisible()
        .catch(() => false);

      if (titleErrorState || descriptionErrorState) {
        console.log(
          `Found fields with error states: Title=${titleErrorState}, Description=${descriptionErrorState}`
        );
        return true;
      }

      // Check for error helper text
      const helperTexts = await this.page.locator('.MuiFormHelperText-root').all();

      for (const helperText of helperTexts) {
        const text = await helperText.textContent();
        const hasError = await helperText
          .evaluate(el => el.classList.contains('Mui-error') || el.classList.contains('error'))
          .catch(() => false);
      ...
      const anyErrorMessage = await this.page
        .getByText('is required', { exact: false })
        .isVisible()
        .catch(() => false);
  ...
  async getFormValues(): Promise<Partial<ICourseCreationData>> {
    ...
    try {
      ...
      for (const selector of categorySelectors) {
        ...
        if (
          (await categoryInput.count()) > 0 &&
          (await categoryInput.isVisible().catch(() => false))
        ) {
      ...
      for (const selector of difficultySelectors) {
        ...
        if (
          (await difficultySelect.count()) > 0 &&
          (await difficultySelect.isVisible().catch(() => false))
        ) {
          formValues.difficulty_level = (await difficultySelect
            .inputValue()
            .catch(() => '')) as any;
      ...
      for (const selector of publishSelectors) {
        ...
        if (
          (await publishSwitch.count()) > 0 &&
          (await publishSwitch.isVisible().catch(() => false))
        ) {
