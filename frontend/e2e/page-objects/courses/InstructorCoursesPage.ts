...
export class InstructorCoursesPage extends BaseCourses {
  ...
  async filterByStatus(status: 'draft' | 'published'): Promise<void> {
    try {
      ...
      await this.page
        .locator(`.MuiMenu-list li:text-is("${status}"), .MuiMenuItem-root:text-is("${status}")`)
        .click();
  ...
  async hasCourse(
    ...
    try {
      ...
      console.log(
        `Looking for course with title: "${title}" (useSearch: ${useSearch}, useFilters: DISABLED, filterStatus: ${filterStatus})`
      );

      // First check if the page has loaded course content
      const hasContent = await this.page
        .locator('[data-testid="courses-list"], .courses-list, .MuiGrid-container, .course-card')
        .isVisible()
        .catch(() => false);
      ...
      const allCourses = await this.page
        .locator('[data-testid*="course"], .course-card, .MuiCard-root')
        .all();
