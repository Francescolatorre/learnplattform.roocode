...
test.describe('Student Dashboard', () => {
  ...
  test.beforeAll(async ({ request }) => {
    ...
    const userResp = await request.get(
      `http://localhost:8000/api/v1/users/?username=${encodeURIComponent(student.username)}`,
      {
    ...
    const studentUser = Array.isArray(userList)
      ? userList[0]
      : userList.results
        ? userList.results[0]
        : null;
    if (!studentUser || !studentUser.id)
      throw new Error('Failed to fetch student user ID from backend');
  ...
  test('shows course progress correctly', async ({ page }) => {
    ...
    for (const courseElement of await page.locator('.course-card').all()) {
      ...
      const progressText = await courseElement
