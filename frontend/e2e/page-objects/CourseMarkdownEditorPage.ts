...
/**
 * Page object for course-related markdown editing.
 * Encapsulates only selectors and logic relevant to course markdown editors.
 */
interface CourseMarkdownSelectors {
    container: string;
    textarea: string;
    writeTab: string;
    previewTab: string;
...
export class CourseMarkdownEditorPage extends BasePage {
    readonly page: Page;
    readonly navigation: NavigationHelper;

    private readonly selectors: CourseMarkdownSelectors = {
        ...
        // Course description selectors ordered by priority (course context)
    ...
    /**
     * Wait for the markdown editor to be fully loaded and ready (course context)
     */
    ...
    /**
     * Enter markdown content into the editor (course context)
     */
    ...
    /**
     * Switch to preview mode (course context)
     */
    ...
    /**
     * Switch to write mode (course context)
     */
    ...
    /**
     * Get current markdown content from the editor (course context)
     */
    ...
    /**
     * Get current content from the editor (course context)
     */
    async getContent(): Promise<string> {
        if (await this.isInPreviewMode()) {
            for (const selector of this.selectors.descriptionSelectors) {
                const element = await this.page.locator(selector).first();
                if (await element.isVisible()) {
                    return await element.innerText();
                }
            }
            throw new Error('No visible course description content found');
        } else {
    ...
    /**
     * Get the rendered HTML from preview (course context)
     */
    async getRenderedHTML(): Promise<string> {
        ...
        throw new Error('Could not find course description preview content');
    }

    /**
     * Check if specific markdown elements are rendered in the preview (course context)
     */
    ...
    /**
     * Open markdown help dialog (course context)
     */
    ...
    /**
     * Close markdown help dialog (course context)
     */
    ...
    /**
     * Check if editor is in write mode (course context)
     */
    ...
    /**
     * Check if editor is in preview mode (course context)
     */
    ...
    /**
     * Wait for preview content to be visible (course context)
     */
    async waitForPreviewContent(timeout: number = 10000): Promise<void> {
        ...
        throw new Error('Course description preview content not visible after timeout');
    }
}
