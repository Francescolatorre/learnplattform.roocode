...
test.describe('Authentication', () => {
  ...
  test('lead instructor can login and access dashboard', async ({ page }) => {
    await login(page, TEST_USERS.lead_instructor.username, TEST_USERS.lead_instructor.password);
  ...
  test('shows loading state during login', async ({ page }) => {
    ...
    const loadingIndicator = page.locator(
      'button[data-testid="login-submit-button"] [role="progressbar"]'
    );
  ...
  test('displays error with invalid credentials', async ({ page }) => {
    ...
    const errorNotification = await page.waitForSelector(
      '.MuiAlert-standardError, .MuiSnackbar-root',
      { timeout: 5000 }
    );
