...
export const fetchStudentProgressByCourse = async (courseId: string, studentId: string, includeDetails = false): Promise<CourseProgress | null> => {
    return apiService.get<CourseProgress>(`courses/${courseId}/student-progress/${studentId}/`, { includeDetails });
};

export const fetchAllStudentsProgress = async (courseId: string): Promise<{ count: number; next: string | null; previous: string | null; results: CourseProgress[] }> => {
    return apiService.get(`courses/${courseId}/student-progress/`);
};

export const getQuizHistory = async (courseId: string, studentId?: string): Promise<QuizHistory[]> => {
    const endpoint = studentId ? `students/${studentId}/quiz-performance/` : `courses/${courseId}/quiz-performance/`;
    return apiService.get<QuizHistory[]>(endpoint);
};

export const getContentEffectivenessData = async (courseId: string): Promise<ContentEffectivenessData> => {
    return apiService.get(`courses/${courseId}/content-effectiveness/`);
};

export const updateTaskProgress = async (courseId: string, taskId: string, progressData: any): Promise<any> => {
    return apiService.patch(`courses/${courseId}/tasks/${taskId}/progress`, progressData);
};

export const submitTask = async (courseId: string, taskId: string, submissionData: any): Promise<any> => {
    return apiService.post(`courses/${courseId}/tasks/${taskId}/submit`, submissionData);
};

export const gradeSubmission = async (courseId: string, taskId: string, studentId: string, gradingData: any): Promise<any> => {
    return apiService.post(`courses/${courseId}/tasks/${taskId}/students/${studentId}/grade`, gradingData);
};
