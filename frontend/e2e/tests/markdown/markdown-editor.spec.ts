...
import { LoginPage, CourseCreationPage, MarkdownEditorPage } from '../../page-objects';
...
test.describe('Markdown Editor Component', () => {
  ...
  test('markdown editor handles basic editing operations', async ({ page }) => {
    ...
    await markdownEditor.enterMarkdown(
  ...
  test('markdown help dialog or documentation is accessible', async ({ page }) => {
    ...
    try {
      ...
      const inlineHelp = await page
        .locator(':has-text("Markdown") :has-text("formatting")')
        .isVisible()
        .catch(() => false);
