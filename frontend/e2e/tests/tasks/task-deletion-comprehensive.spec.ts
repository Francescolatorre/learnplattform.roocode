...
test.describe('Task Deletion Feature - Comprehensive Tests', () => {
  ...
  test('delete functionality works on course details page (/instructor/courses/{id})', async ({
    page,
  }) => {
    ...
    if (await createTaskButton.isVisible()) {
      ...
      if (infoButtonCount > 0) {
        console.log(
          `Found ${infoButtonCount} disabled delete buttons (info icons) on course details page`
        );
      ...
      console.log(
        'Course details page does not show tasks inline - may use separate task management page'
      );
    }
  });

  test('delete functionality works on dedicated tasks page (/instructor/courses/{id}/tasks)', async ({
    page,
  }) => {
    ...
    if (infoButtonCount > 0) {
      console.log(
        `Found ${infoButtonCount} disabled delete buttons (info icons) on dedicated tasks page`
      );
  ...
  test('verify both pages show consistent delete button behavior', async ({ page }) => {
    ...
    console.log(
      `Course details page: ${detailsDeleteCount} delete buttons, ${detailsInfoCount} info buttons`
    );
