...
const PerformanceAnalysisView: React.FC<PerformanceAnalysisViewProps> = ({
    ...
    const tasksWithScores = taskProgress.filter(task =>
        task.score !== null &&
        task.status === 'graded'
    );

    // Group tasks by type
    const tasksByType = tasksWithScores.reduce((acc, task) => {
    ...
    const prepareSkillProficiencyData = () => {
        ...
        const skills = ['Problem Solving', 'Critical Thinking', 'Communication', 'Technical Knowledge', 'Creativity'];
    ...
    const preparePerformanceOverTimeData = () => {
        ...
        const sortedTasks = [...tasksWithScores].filter(task => task.submissionDate)
            .sort((a, b) => {
    ...
    const skillProficiencyData = prepareSkillProficiencyData();
