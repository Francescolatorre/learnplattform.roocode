...
`;
  ...
  ): Promise<IMarkdownResult> {
    // Changed return type to match new implementation
   ...
   */
  static async verifySanitization(page: Page, containerSelector?: string): Promise<void> {
    const selectors = containerSelector ? [containerSelector] : this.markdownContainerSelectors;
  ...
  private static async countMarkdownElements(
    page: Page,
    container: string
